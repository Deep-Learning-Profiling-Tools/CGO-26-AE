...
PROFILING_OPTIONS = [
    ...
    ("rocprof", [])  # Special case, will be handled differently
]


def run_command(cmd: List[str], output_file: str, use_rocprof: bool = False, dry_run: bool = False) -> bool:
    """Run a command and save output to file."""
    if use_rocprof:
        # Prepend rocprof command
        rocprof_cmd = [
            "rocprof",
            "--hip-trace",
            "--hsa-trace",
            "-o", output_file.replace(".log", ".csv")
        ]
        cmd = rocprof_cmd + cmd
...
def run_model_experiment(model_file: str, model_runners_dir: Path, output_dir: Path, dry_run: bool = False) -> Tuple[Dict[str, Optional[float]], Dict[str, Optional[int]]]:
    ...
    for option_name, option_args in PROFILING_OPTIONS:
        ...
        if option_name == "rocprof":
            # Special handling for rocprof
            success = run_command(
        ...
        if not dry_run and option_name in ["torch", "proton", "rocprof"]:
            profiling_file = None
            ...
            elif option_name == "rocprof":
                profiling_file = output_dir / f"{model_base_name}_rocprof.csv"
...
def print_table(results: Dict[str, Dict[str, Optional[float]]]):
    """Print results table."""
    # Headers
    headers = ["Name", "mi300x-baseline", "mi300x-torch", "mi300x-proton", "mi300x-rocprof"]
    ...
    for model_name in results:
        ...
        for option in ["baseline", "torch", "proton", "rocprof"]:
            value = results[model_name].get(option)
            col_idx = headers.index(f'mi300x-{option}')
...
def main():
    ...
    output_dir = script_dir / "experiment_outputs_amd"
    ...
    if not args.dry_run:
        ...
        results_file = output_dir / "experiment_results_amd.json"
        with open(results_file, "w") as f:
            json.dump(all_results, f, indent=2)
        print(f"\nResults saved to: {results_file}")

        # Save file sizes to JSON
        file_sizes_file = output_dir / "experiment_results_size_amd.json"
