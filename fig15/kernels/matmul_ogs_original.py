...
def simple_matmul_ogs_test(use_cuda_event: bool = False):
    """Simple test function for profiling matmul_ogs"""
    ...
    for _ in range(5):
        try:
            _ = matmul_ogs(x, w, bias)
        except opt_flags.InapplicableConstraint:
            print("Warning: Inapplicable constraint during warmup")
            break
    ...
    try:
        ...
        print("Error: Inapplicable constraint in main computation")
        return None
    if use_cuda_event:
        end_event.record()
...
if __name__ == "__main__":
    ...
    if args.simple or args.profile:
        if args.profile:
            ...
            result = simple_matmul_ogs_test(use_cuda_event=False)
            proton.finalize()
